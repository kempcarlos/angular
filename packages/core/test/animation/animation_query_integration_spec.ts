 ...
 */
import {AUTO_STYLE, AnimationPlayer, animate, animateChild, query, queryAll, stagger, state, style, transition, trigger, ɵAnimationGroupPlayer as AnimationGroupPlayer} from '@angular/animations';
  ...
  describe('animation query tests', function() {
    ...
    describe('query() and queryAll()', () => {
               ...
               trigger(
                   ...
                   [
                     transition(
                         '* => *',
                         [
                           queryAll('@*:animating', [animate(1000, style({background: 'red'}))]),
                         ]),
                   ]),
               trigger(
                   'child',
      ...
      it('should collect styles for the same elements between queries', () => {
        @Component({
          ...
          animations: [
            trigger('myAnimation', [
              transition('* => go', [
                queryAll('&, header, footer', style({opacity: '0.01'})),
                animate(1000, style({opacity: '1'})),
        ...
        [p1, p2, p3].forEach(p => {expect(p.keyframes).toEqual([
                               {opacity: '0.01', offset: 0}, {opacity: '0.01', offset: 1}
                             ])});

        [p4, p5, p6].forEach(p => {expect(p.keyframes).toEqual([
                               {opacity: '0.01', offset: 0}, {opacity: '1', offset: 1}
                             ])});

        expect(p4.delay).toEqual(1);
        expect(p4.duration).toEqual(1000);
        expect(p5.delay).toEqual(1001);
        expect(p5.duration).toEqual(1000);
        expect(p6.delay).toEqual(1501);
        expect(p6.duration).toEqual(1000);
      });

      it('should retain style values when & is used inside of a query', () => {
        @Component({
          selector: 'ani-cmp',
          template: `
          ...
          animations: [trigger(
              ...
              [transition(
                  '* => go',
                  [query('&', style({opacity: '0.5'})), animate(1000, style({opacity: '1'}))])])]
        })
        class Cmp {
          public exp: any;
        }
        ...
        @Component({
          ...
          animations: [
            ...
            trigger(
                'child',
                [
                  state('*, void', style({height: '0px'})), state('b', style({height: '444px'})),
      ...
      it('should find newly inserted items in the component via :enter', () => {
        ...
        const [p1, p2, p3] = players;
        expect(p1.element.innerText).toEqual('0');
        expect(p2.element.innerText).toEqual('1');
        expect(p3.element.innerText).toEqual('2');
          ...
          animations: [trigger(
              ...
              [
                transition(
                    'a => b',
                    [queryAll(
                        ':leave', [style({opacity: 1}), animate(1000, style({opacity: .5}))])]),
              ])]
        })
        class Cmp {
          public exp: any;
      ...
      it('should properly cancel items that were queried into a former animation', () => {
        @Component({
          ...
          animations: [trigger(
            ...
            [
              transition('* => on', [
                queryAll(':enter', [style({opacity: 0}), animate(1000, style({opacity: 1}))]),
                queryAll(':enter', [style({width: 0}), animate(1000, style({height: 200}))])
              ]),
              transition('* => off', [
                queryAll(':leave', [animate(1000, style({width: 0}))]),
                queryAll(':leave', [animate(1000, style({opacity: 0}))])
              ]),
            ])]
        })
        class Cmp {
    ...
    describe('sub triggers', () => {
             ...
             animations: [
               trigger('parent', [transition(
                                     ...
                                     [
                                       ...
                                       queryAll('.item', [animateChild({ duration: '2.5s', delay: '500ms' })]),
                                       animate(1000, style({opacity: '0'}))
                                     ])]),
               trigger('child', [transition(
                                    ':enter',
        ...
        @Component({
          ...
          animations:
              [trigger('parent', [transition(
                                     ...
                                     [
                                       style({opacity: 0}), animate(1000, style({opacity: 1})),
                                       queryAll('.child', [animateChild({duration: '1s'})]),
                                       animate(1000, style({opacity: 0}))
                                     ])])]
        })
        class Cmp {
        ...
        @Component({
          ...
          animations: [
            ...
            trigger('parent', [transition(
                                  ...
                                  [
                                    style({opacity: 0}), animate(1000, style({opacity: 1})),
                                    queryAll('.child', [animateChild({duration: '0'})]),
                                    animate(1000, style({opacity: 0}))
                                  ])])
          ]
        })
               ...
               trigger(
                   ...
                   [
                     transition(
                         '* => enter',
                         [
                           queryAll(':enter', [animate(1000, style({color: 'red'}))]),
                         ]),
                     transition(
                         '* => leave',
                         [
                           queryAll(':leave', [animate(1000, style({color: 'blue'}))]),
                         ]),
                   ]),
             ],
             template: `
         ...
         fakeAsync(() => {
           @Component({
             ...
             animations: [
               ...
               trigger(
                   'childWithAnimation',
                   [transition(':leave', [animate(1000, style({background: 'red'}))])])
             ],
             template: `
          ...
          `
           ...
           expect(cmp.log).toEqual(
               ['p-start', 'c1-start', 'c1-done', 'c2-start', 'c2-done', 'c3-start', 'c3-done']);
           cmp.log = [];
           flushMicrotasks();

           expect(cmp.log).toEqual(['p-done']);
         }));
         ...
         () => {
           ...
           const childPlayer = groupPlayer.players.find(player => {
             if (player instanceof MockAnimationPlayer) {
               return player.element.matches('.child');
             }
               ...
               trigger(
                   ...
                   [
                     transition(
                         '* => go',
                         [
                           queryAll('&, @grandChildAnimation', style({opacity: 0})),
                           animate(1000, style({opacity: 1})),
                           query(
                               '@grandChildAnimation',
                               [
      ...
      it('should emulate enter and leave animations on the nearest homeElements when inserted dynamically');
    });
  });
}
